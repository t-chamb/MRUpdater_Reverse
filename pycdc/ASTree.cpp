...
PycRef<ASTNode> BuildFromCode(PycRef<PycCode> code, PycModule* mod)
{
    ...
    while (!source.atEof()) {
        ...
        case Pyc::JUMP_IF_NOT_EXC_MATCH_A:
            {
                // Used in exception matching - pops TOS (exception type to match)
                // and TOS1 (exception instance), jumps if they don't match
                stack.pop(); // Exception type to match against
                // Leave the exception instance on stack for the handler
            }
            break;
        case Pyc::RERAISE_A:
            {
                // RERAISE: Re-raises the exception currently being handled
                // No stack manipulation needed as the exception is already set
            }
            break;
        default:
