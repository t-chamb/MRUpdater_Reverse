...
from botocore.exceptions import InvalidMaxRetryAttemptsError, InvalidRetryConfigurationError, InvalidRetryModeError, InvalidS3AddressingStyleError
...
class Config:
    ...
    def inject_host_prefix(self):
        if self._inject_host_prefix == 'UNSET':
            return True
        return None._inject_host_prefix

    inject_host_prefix = property(inject_host_prefix)
    
    def inject_host_prefix(self, value):
        self._inject_host_prefix = value

    inject_host_prefix = inject_host_prefix.setter(inject_host_prefix)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key, value in config_vars.items():
            if key == 'inject_host_prefix' and 'inject_host_prefix' not in self._user_provided_options.keys():
                continue
    ...
    def _validate_s3_configuration(self, s3):
        if s3 is not None:
            ...
            if addressing_style not in ('virtual', 'auto', 'path', None):
                raise InvalidS3AddressingStyleError(addressing_style, **('s3_addressing_style',))
        return None
        ...
        if retries is not None:
            for key, value in retries.items():
                if key not in valid_options:
                    raise InvalidRetryConfigurationError(key, valid_options, **('retry_config_option', 'valid_options'))
                if key == 'max_attempts' and value < 0:
                    raise InvalidMaxRetryAttemptsError(value, 0, **('provided_max_attempts', 'min_value'))
                if key == 'total_max_attempts' and value < 1:
                    raise InvalidMaxRetryAttemptsError(value, 1, **('provided_max_attempts', 'min_value'))
                if key == 'mode' and value not in valid_modes:
                    raise InvalidRetryModeError(value, valid_modes, **('provided_retry_mode', 'valid_modes'))
        return None
